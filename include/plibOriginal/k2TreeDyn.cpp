...
long long int numBytes() {
    ...
    bytes += sizeof(int)*first.size();
    for (int i = 0; i < next.size();i++)
        bytes += sizeof(int)*next[i].size();
    return bytes;
}
    ...
    if (this->tl.at(pos)){
        if (!this->multi[place]) {
            ...
            if (posMulti >= next.size())
                ...
                next.insert(next.begin()+posMulti,std::vector<int>());
            next[posMulti].push_back(first[place]);
            next[posMulti].push_back(edgeId);
            first[place] = 0;
        }else {
            int posMulti = multi.rank1(place);
            next[posMulti].push_back(edgeId);
        ...
        if (first.begin()+place >= first.end())
            first.push_back(edgeId);
        else
            first.insert(first.begin() + place, edgeId);
    }
}
                    ...
                    if (!multiEdge) {
                        ...
                        if (multi.at(placeInMulti)){
                            ...
                            for (int z = 0; z < next[posMulti].size(); z++)
                                neighbors[pos + i - nodes[0] % k].push_back(next[posMulti][z]);
                        }else {
                            neighbors[pos+i-nodes[0]%k].push_back(connected[0] + j - connected[0] % k);
                            neighbors[pos + i - nodes[0] % k].push_back(1);
                            neighbors[pos + i - nodes[0] % k].push_back(first[placeInMulti]);
                        }
                    }
                }
    }
