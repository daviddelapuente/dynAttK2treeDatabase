...
int binarySearch(std::string label, std::vector<data> attValsSorted){
    ...
    if (posSup > 0) {
        trust = label.compare(*attValsSorted[pos].s);
        while (trust) {
            posPiv = pos;
            ...
            if (posPiv == pos || pos == posSup || pos == posInf) {
                if (pos == attValsSorted.size())
                    ...
                    return pos+(label > *attValsSorted[pos].s);
            }
...
public:
    ...
    void addAtt(std::string val, int node, bool insert){
        attVals.push_back(string(val,val.size()));
        data a = {new string(attVals[node]), node};
    ...
    void printFullAttSortedList(){
        for (int i = 0; i < attVals.size(); i++)
            printf("%s ", attValsSorted[i].s->c_str());
        printf("\n");
    }
    ...
    std::vector<int> getNodesWithVal(string attVal){
        ...
        if (pos == attValsSorted.size() && attValsSorted[pos-1].s->compare(attVal))
            return output;
        if (pos == attValsSorted.size())
            pos--;
        if (attValsSorted[pos].s->compare(attVal))
            return output;
        ...
        for (int i = -1; i < 2; i+=2) {
            pos = posPiv+i;
            while (true) {
                if ((pos) < 0 || (pos) >= (attValsSorted.size()) || attValsSorted[pos].s->compare(attVal))
                    break;
                output.push_back(attValsSorted[pos].node);
                pos+=i;
            }
    ...
    void saveValues(char * fileName){
        ...
        if (fs.is_open()) {
            ...
            for (int i = 0; i < attValsSorted.size(); i++)
                fs << attValsSorted[i].s->c_str()<<'|'<<attValsSorted[i].node << "\t";
        }else
            printf("Cerrado\n");
    ...
    long long int numBytes(){
        ...
        for (int i = 0; i < attVals.size(); i++)
            bytes += attVals[i].size();
        return 5*attValsSorted.size()+bytes;
    }
private:
    std::vector<std::string>  attVals;
