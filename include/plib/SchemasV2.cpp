...
public:
    ...
    int add(vector<string> info, vector<int> attType){
        ...
        if (pos == nodes.size() || info[0].compare(nodes[pos].labelName)!=0) {
            ...
            (pos == nodes.size())? nodes.push_back(nod) : (void)nodes.insert(nodes.begin()+pos+info[0].compare(nodes[pos].labelName), nod);
    ...
    uint64_t numBytesUsed2(){
        ...
        for (int i = 0; i < nodes.size(); i++){
            ...
            for (int j = 0; j < nodes[i].attributes.size();j++)
                if (nodes[i].attributes[j] != NULL)
                    bytes += nodes[i].attributes[j]->numBytes2()+ sizeof(Att);
        }
        for (int i = 0; i < labels.size(); i++)
            bytes += labels[i].size();
        return bytes+wtSimbols.bit_size()/8;
